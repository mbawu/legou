...
//		String email = emailTxt.getText().toString();
		...
		// 打印生成的链接地址
		MyApplication.printLog("PersonRegister", request + "-->Url:"
				+ MyApplication.getUrl(paramter, url));
		/*
		 * 向服务器发送请求
		 */
		MyApplication.client.postWithURL(url, paramter,
				new Listener<JSONObject>() {
					public void onResponse(JSONObject response) {
						try {
							MyApplication.printLog("PersonRegister", request
									...
									&& code == 1)// 执行验证验证码操作
							{
								register();// 成功验证以后再去执行注册操作
									...
									&& code == 1) {// 执行获取验证码操作
								Toast.makeText(PersonRegister.this, "验证码已发送",
										2000).show();
								...
								String result = ErrorMsg.getErrorMsg(request,
										code);
	...
	private void register() {
		...
		final String pwd = pwdTxt.getText().toString();
		String pwdAgain = pwdAginTxt.getText().toString();
		if (phone.equals("") || pwd.equals("") || pwdAgain.equals("")) {
			Toast.makeText(this, "请输入完整注册信息！", 2000).show();
			return;
		}
		if (!pwd.equals(pwdAgain)) {
			Toast.makeText(this, "两次输入的密码不一致", 2000).show();
			return;
		}
		// 注册
		HashMap<String, String> paramter = new HashMap<String, String>();
		paramter.put("act", "register2");
		paramter.put("sid", MyApplication.sid);
		paramter.put("username", phone);
		...
		MyApplication.printLog("PersonRegister",
				"Loginurl:" + MyApplication.getUrl(paramter, Url.URL_USERS));
					...
					public void onResponse(JSONObject response) {
						try {
							...
							if (code == 1) {
								 ...
								 */
								MyApplication.ed.putString("username", phone);
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (seconds == msg.arg1) {
				...
				getMsgBtn.setBackgroundDrawable(PersonRegister.this
